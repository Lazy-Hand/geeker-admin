...
export interface ProTableProps {
  ...
  /**
   * @name data
   * @description 静态 table data 数据，若存在则不会使用 requestApi 返回的 data ==> 非必传
   * @type {any[]}
   */
  data?: any[];
  ...
  /**
   * @name requestAuto
   * @description 是否自动执行请求 api ==> 非必传（默认为true）
   * @type {boolean}
   * @default true
   */
  requestAuto?: boolean;
  ...
  /**
   * @name dataCallback
   * @description 返回数据的回调函数，可以对数据进行处理 ==> 非必传
   * @type {(data: any) => any}
   */
  ...
  /**
   * @name title
   * @description 表格标题，目前只在打印的时候用到 ==> 非必传
   * @type {string}
   */
  title?: string;
  ...
  /**
   * @name pagination
   * @description 是否需要分页组件 ==> 非必传（默认为true）
   * @type {boolean}
   * @default true
   */
  pagination?: boolean;
  ...
  /**
   * @name border
   * @description 是否带有纵向边框 ==> 非必传（默认为true）
   * @type {boolean}
   * @default true
   */
  border?: boolean;
  ...
  /**
   * @name toolButton
   * @description 是否显示表格功能按钮 ==> 非必传（默认为true）
   * @type {boolean}
   * @default true
   */
  toolButton?: boolean;
  ...
  /**
   * @name rowKey
   * @description 行数据的 Key，用来优化 Table 的渲染，当表格数据多选时，所指定的 id ==> 非必传（默认为 id）
   * @type {string}
   * @default id
   */
  rowKey?: string;
  ...
  /**
   * @name searchCol
   * @description 表格搜索项 每列占比配置 ==> 非必传 { xs: 1, sm: 2, md: 2, lg: 3, xl: 4 }
